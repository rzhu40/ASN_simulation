...
def draw_plotly(toDraw, PathHighlight=None, JunctionsToObserve=[], **kwargs):
    ...
    #     PathInd = []
    PathInd = []
    if PathHighlight == 'first':
        from analysis.GraphTheory import findCurrent
        try:
            PathInd = findCurrent(toDraw)[0][0]
        except:
            print('No current path formed in this simulation. No path will be highlighted')
    elif PathHighlight == 'shortest':
        try:
            PathInd = toDraw.shortestPaths[0]
        except:
            print('No certain shortest path for the drew part.')
    elif (PathHighlight == None)or(PathHighlight == 'none'):
        PathInd = []
    else:
        print('Highlight type error. Will not highlight any paths.')
