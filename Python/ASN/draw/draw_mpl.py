...
def draw_mpl(toDraw, ax = None, **kwargs):
    ...
    ax.set_title(f'time = {np.round(toDraw.TimeVector[this_TimeStamp], 3)}')
    ...
    Plot currents
    """
    sources = toDraw.sources
    drains = toDraw.drains
    ...
    adjMat = toDraw.connectivity.adj_matrix
    edgeList = toDraw.connectivity.edge_list
    current = toDraw.junctionVoltage[this_TimeStamp,:]*toDraw.junctionConductance[this_TimeStamp,:]
    ...
    if not hasattr(toDraw, 'onCurrentPath'):
        toDraw.isOnCurrentPath()
    
    onPathChecker = np.zeros(toDraw.numOfWires, dtype = bool)
    ...
    for this_wire in range(xc.size):
        ...
        onPathChecker[this_wire] = np.sum(toDraw.onCurrentPath[this_TimeStamp, contactJunctions])
        ...
        if this_wire in np.append(sources, drains):
            ...
            arrowX = np.append(arrowX, np.cos(wireAngles[this_wire]) * totalCurrent)
            arrowY = np.append(arrowY, np.sin(wireAngles[this_wire]) * totalCurrent)
    ...
    rgba[:,0] = 1
    rgba[:,3] = 0.7
    arrow_scale = abs(totalCurrent/toDraw.connectivity.length_x)*10
    ax.quiver(arrowCenterX, arrowCenterY, 
            arrowX , arrowY, 
            color = rgba, 
            width = 0.005,
